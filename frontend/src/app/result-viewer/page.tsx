...
export default function ViewResultPage() {
  ...
  //2 khai báo để sử dụng cho việc lấy orderID từ URL. VD: https://result-viewer-zeta.vercel.app/?orderId=12345
  const searchParams = useSearchParams();
  const orderID = searchParams.get("orderID") || "";
  // Sử dụng orderID trong fetch API nếu cần thiết
  /*
  useEffect(() => {
    if (!orderId) return;

    fetch(`/api/result?orderId=${orderId}`)
      .then(async (res) => {
  ...
  */
    // // Lấy dữ liệu với orderID trong URL với orderID là tên file lưu trong thư mục public/results
    // useEffect(() => {
    //   if (!orderID) return;

    //   fetch(`/results/${orderID}`)
    //     .then(async (res) => {
    //       if (!res.ok) throw new Error("Không thể tải kết quả");
    //       return res.json();
    //     })
    ...
    //get dữ liệu từ thư mục api/route.ts
  useEffect(() => {
    fetch("/api/result")
      .then(async (res) => {
        if (!res.ok) throw new Error("Không thể tải kết quả");
